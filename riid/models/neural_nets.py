...
class SimpleLabelProportionEstimator(TFModelBase):
    METRICS = {
    ...
    def _get_model_file_paths(self, dir_path, model_format):
        model_info_path = os.path.join(dir_path, "model_info.json")
    ...
    def _forward_pass(self, spectra, sources, training=False):
        ...
        if training:
            ...
            #   to tape.gradient()
            grads = tape.gradient(semisup_losses, self.model.trainable_weights)
    ...
    def fit(self, fg_seeds_ss: SampleSet,
            ss: SampleSet,
        ...
        Args:
            fg_seeds_ss: a sampleset of pure, long-collect foreground spectra.
    ...
    def save(self, dir_path, save_format=None):
        model_info_path, model_path = \
            self._get_model_file_paths(dir_path, save_format)
        ...
        # save train and val histories
        with open(os.path.join(dir_path, 'train_history.pkl'), 'wb') as th:
            pkl.dump(self.train_history, th)
        with open(os.path.join(dir_path, 'validation_history.pkl'), 'wb') as vh:
            pkl.dump(self.val_history, vh)

    def load(self, dir_path, save_format=None):
        model_info_path, model_path = \
            self._get_model_file_paths(dir_path, save_format)
        ...
        # load train and val histories
        with open(os.path.join(dir_path, 'train_history.pkl'), 'rb') as th:
            self.train_history = pkl.load(th)
        with open(os.path.join(dir_path, 'validation_history.pkl'), 'rb') as vh:
            self.val_history = pkl.load(vh)
